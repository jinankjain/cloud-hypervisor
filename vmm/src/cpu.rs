...
impl CpuManager {
    ...
    pub fn new(
        ...
        #[cfg(target_arch = "x86_64")]
        let cpuid = {
            let phys_bits = physical_bits(inner_cpu_manager.config.max_phys_bits);
            arch::generate_common_cpuid(
                hypervisor,
                inner_cpu_manager.config
                    .topology
                    .clone()
                    .map(|t| (t.threads_per_core, t.cores_per_die, t.dies_per_package)),
                sgx_epc_sections,
                phys_bits,
                inner_cpu_manager.config.kvm_hyperv,
                #[cfg(feature = "tdx")]
                tdx_enabled,
            )
        ...
        #[cfg(target_arch = "x86_64")]
        if inner_cpu_manager.config.features.amx {
        ...
        let affinity = if let Some(cpu_affinity) = inner_cpu_manager.config.affinity.as_ref() {
            cpu_affinity
                .iter()
